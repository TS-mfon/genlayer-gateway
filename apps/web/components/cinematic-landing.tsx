...
import { ArrowUpRight, Check, ChevronRight, Code2, Menu, X } from "lucide-react";
    ...
    <main className={`cinematic-landing ${open ? "is-open" : ""}`}>
      ...
      <header className="cinematic-topbar">
        ...
        <nav className="cinematic-links" aria-label="Primary navigation">
          <a href="#how-it-works">How it works</a><a href="#developers">For developers</a><Link href="/routes">Routes</Link><Link href="/docs/overview">Docs</Link>
        </nav>
        <Link className="cinematic-pill cinematic-pill-nav" href="/test-console">Run the demo <ArrowUpRight size={15} /></Link>
      ...
      <nav id="cinematic-menu" className="cinematic-menu" aria-label="Mobile navigation" aria-hidden={!open}>
        <div className="cinematic-menu-inner"><span className="cinematic-menu-eyebrow">Gateway / Menu</span><div className="cinematic-menu-list"><a tabIndex={menuTabIndex} href="#how-it-works" onClick={closeMenu}>How it works <ChevronRight /></a><a tabIndex={menuTabIndex} href="#developers" onClick={closeMenu}>For developers <ChevronRight /></a><Link tabIndex={menuTabIndex} href="/routes" onClick={closeMenu}>Routes <ChevronRight /></Link><Link tabIndex={menuTabIndex} href="/docs/overview" onClick={closeMenu}>Docs <ChevronRight /></Link></div><div className="cinematic-menu-foot"><Link tabIndex={menuTabIndex} className="cinematic-pill" href="/test-console" onClick={closeMenu}>Run the demo <ArrowUpRight size={15} /></Link><Link tabIndex={menuTabIndex} className="cinematic-ghost" href="/docs/quickstart" onClick={closeMenu}>View integration guide</Link></div></div>
      </nav>
      <section className="cinematic-hero" aria-labelledby="gateway-headline">
        ...
        <div className="cinematic-actions"><Link className="cinematic-pill cinematic-pill-hero" href="/test-console">Run the demo <ArrowUpRight size={16} /></Link><Link className="cinematic-ghost" href="/playground">View developer integration <Code2 size={16} /></Link></div>
        <div className="cinematic-role-entry" aria-label="MVP demo role entry points"><span>MVP DEMO ROLES</span><Link href="/create-job">Client / Post a job <ArrowUpRight size={13} /></Link><Link href="/dashboard/worker">Worker / Browse jobs <ArrowUpRight size={13} /></Link></div>
      </section>
      ...
      <section className="cinematic-developer-note" id="developers"><span className="cinematic-kicker">BUILT FOR DEVELOPERS</span><strong>Integrate judgment without migrating your application.</strong><p>Start with the verified Base Sepolia route. Add your own GenLayer contract through a reviewed route profile, typed adapter, and explicit result schema.</p><Link className="cinematic-inline-link" href="/docs/routes">Learn the route model <ArrowUpRight size={14} /></Link></section>
