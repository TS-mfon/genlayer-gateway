...
contract LayerZeroGatewaySenderV2 is ITransportAdapter {
    ...
    constructor(address endpoint_, address owner_, uint32 remoteEid_, bytes32 remoteReceiver_) {
        if (endpoint_ == address(0) || owner_ == address(0) || remoteReceiver_ == bytes32(0)) {
    ...
    function setRouter(address router_) external onlyOwner {
        ...
        router = GatewayRouterV2(router_);
    }
    ...
    function _decode(bytes calldata payload)
        private
        pure
        returns (bytes32 requestId, address destination)
    {
        (, requestId,,,,,,,, destination,,,,,) = abi.decode(
            payload,
            (
                uint8,
                bytes32,
    ...
    function quoteDispatch(bytes calldata payload) external view returns (uint256) {
        ...
        bytes memory message =
            abi.encode(uint32(block.chainid), address(router), destination, payload, bytes32(0));
        return endpoint.quote(
            MessagingParams(remoteEid, remoteReceiver, message, options, false), address(this)
        )
        .nativeFee;
    }
    ...
    function dispatch(bytes calldata payload) external payable returns (bytes32 messageId) {
        ...
        bytes memory message =
            abi.encode(uint32(block.chainid), address(router), destination, payload, messageId);
